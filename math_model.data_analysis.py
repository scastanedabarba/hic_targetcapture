...
df = pd.DataFrame(columns = ['Library', 'Depth', 'Trial', 'pB10_pputida', 'pB10_ecoli', 'ecoli_ecoli', 'pb10_pb10', 'pputida_pputida'])

os.chdir('/mnt/ceph/cast9836/00_projects/hic_targetcapture/00_scripts/')
files = glob.glob('../05_bwa_alignment/subsampled/*.bam')
files.sort()
...
for file in files:
    ...
    df = df.append({'Library': index, 'Depth':depth, 'Trial':trial, 'pB10_pputida':counts[0], 
                    'pB10_ecoli':counts[1], 'ecoli_ecoli':counts[2], 'pb10_pb10':counts[3],
                    'pputida_pputida':counts[4]}, ignore_index=True)
    
    percent = (lib/total)*100            
    print('Library ', index, ' at ', depth, ' sequencing depth and trial ', trial, ' done ::: ', round(percent,2))
    
    df.to_csv("../06_output/read_simulations.csv", index=False)